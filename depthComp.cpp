...
Mat DepthComp::preProcess(Mat depthIn, const Mat &labelRszd, bool depthNormalize)
{
	...
	for (int i = 0; i < (depthIn.rows); i++)
	{ // loop over the rows
		for (int j = 1; j < (depthIn.cols); j++)
		{ // loop over the column

			if ((depthIn.at<dtype>(i, j + 1) == depthIn.at<dtype>(i, j - 1)) && (labelRszd.at<dtype>(i, j + 1) == labelRszd.at<dtype>(i, j - 1)) &&
				(depthIn.at<dtype>(i, j + 1) != 0) && (depthIn.at<dtype>(i, j + 1) != depthIn.at<dtype>(i, j)))
			{
				depthIn.at<dtype>(i, j) = 0;
			}
			if ((depthIn.at<dtype>(i, j + 2) == depthIn.at<dtype>(i, j - 1)) && (labelRszd.at<dtype>(i, j + 2) == labelRszd.at<dtype>(i, j - 1)) &&
				(depthIn.at<dtype>(i, j + 2) != 0) && (depthIn.at<dtype>(i, j + 2) != depthIn.at<dtype>(i, j)) && (depthIn.at<dtype>(i, j) != 0) && (depthIn.at<dtype>(i, j + 1) != 0))
			{
				depthIn.at<dtype>(i, j) = 0;
				depthIn.at<dtype>(i, j + 1) = 0;
			}
...
Mat DepthComp::postProcess(Mat depthIn, const Mat &labelRszd, bool inpaintLeftOvers)
{
	...
	{ // loop over the rows
		...
		{ // loop over the column
			...
			if (depthIn.at<dtype>(i, j) == 255)
			{
				depthIn.at<dtype>(i, j) = 0;
			}
			if (depthIn.at<dtype>(i, j) == 0)
			{

				mask.at<dtype>(i, j) = 255;
...
Mat DepthComp::identFillHoles(Mat depthIn, Mat labelRszd, bool logStats)
{
		   ...
		   ((times == 2) && (holesExist == 2)))
	{
		...
		if (myfile.is_open())
		{
			...
			myfile << "\nIn the image, the number of ROWS is :  "
		...
		for (int i = 0; i < (depthIn.rows); i++)
		{ // for loop checking rows for holes
			for (int j = 0; j < (depthIn.cols); j++)
			{ // for loop checking columns for holes

				if (depthIn.at<dtype>(i, j) == 0)
				{ // checking to see if holes remain

					if (times == 1)
					{ // the thing has been run ONCE and now we have to flip it
					...
					{ // the thing has been run TWICE and flipped already so we now flip it back
					...
					{ // the thing has been run THREE times and flipped twice already
	...
	nestBreak:
		...
		if ((times == 1) && (holesExist == 0))
		{ // this means it has been run once and all holes are FILLED before any flipping
		...
		else if ((times == 3) && (holesExist == 3))
		{ // this means it has been run THREE and holes still exist
...
Mat DepthComp::identFillHolesPass(Mat depthIn, const Mat &labelRszd)
{
	...
	{ // loop over the rows
		...
		{ // loop over the columns
			...
			if (depthIn.at<dtype>(i, j) == 0)
			{ // we have reached a hole in a line
				...
				if (begin1 != 0)
				{ // when the hole does NOT begin1 in the begin1ning of the line
					while ((labelRszd.at<dtype>(i, j) == labelRszd.at<dtype>(i, (j + 1))) && (depthIn.at<dtype>(i, j) == 0) && (j != (depthIn.cols - 1)))
					{
						j++;
					} // end1 of while

					if ((j == (depthIn.cols - 1)) || (labelRszd.at<dtype>(i, j) != labelRszd.at<dtype>(i, j + 1)))
					{
						// either the holes extend1s to the end1 of the line OR the object has end1ed but the hole has not.

						if (depthIn.at<dtype>(i, j) == 0)
						{ //when we have stopped on the hole
							end1 = j;
							hLen = end1 - begin1 + 1;

							if ((begin1 - hLen) > 0)
							{ // when there is enough information on the left of the hole
								if (labelRszd.at<dtype>(i, ((begin1 - hLen) - 1)) == labelRszd.at<dtype>(i, begin1))
								{ // and more than enough information on the left is in the same object
									caseNo = 1;
									case1++;

								} // end1 of if for case 1
								else if (labelRszd.at<dtype>(i, ((begin1 - hLen) - 1)) != labelRszd.at<dtype>(i, begin1) &&
										 labelRszd.at<dtype>(i, (begin1 - hLen)) == labelRszd.at<dtype>(i, begin1))
								{
								...
								else if (labelRszd.at<dtype>(i, (begin1 - hLen)) != labelRszd.at<dtype>(i, begin1))
								{ // but ALL the information is NOT in the same object

									if (labelRszd.at<dtype>(i, (begin1 - 1)) == labelRszd.at<dtype>(i, begin1))
									{ // there is at least one pixel on the left in the same label
										caseNo = 3;
										case3++;
									} // end1 of if for case 3
									else if (labelRszd.at<dtype>(i, (begin1 - 1)) != labelRszd.at<dtype>(i, begin1))
									{ // there are NO pixels on the left in the same label
										caseNo = 12;
										case12++;
							...
							else if ((begin1 - hLen) < 0)
							{ //when there is NOT enough information on the left but some
							...
							{ // when the amount of information on the left is EXACTLY the same as the length of the hole

								if (labelRszd.at<dtype>(i, (begin1 - hLen)) == labelRszd.at<dtype>(i, begin1))
								{ // there is EXACTLY the same amount of info in the same label on the left
									caseNo = 2;
									case2++;
								} //end1 of if for case 2
								else if (labelRszd.at<dtype>(i, (begin1 - hLen)) != labelRszd.at<dtype>(i, begin1))
								{ // there is NOT enough information on the left in the same label
						...
						else if (depthIn.at<dtype>(i, j) != 0)
						{ //when the hole does NOT continue to the end1 of the line, but to one pixel before the end1
							end1 = j - 1;
							hLen = end1 - begin1 + 1;
							...
							{ // when there is enough information on the left of the hole
								...
								if ((labelRszd.at<dtype>(i, ((begin1 - hLen) - 1)) == labelRszd.at<dtype>(i, begin1)) && (labelRszd.at<dtype>(i, ((begin1 - 1))) == labelRszd.at<dtype>(i, begin1)))
								{ // and more than enough information on the left is in the same object
									...
									{
										if ((labelRszd.at<dtype>(i, begin1 - hLen + chindex) != labelRszd.at<dtype>(i, begin1)) || (depthIn.at<dtype>(i, begin1 - hLen + chindex) == 0))
										{
											checkLeftLabel = 0;
										}
									}
									...
									else if (checkLeftLabel == 0)
									{
										...
										else
										{
										 ...
										 (labelRszd.at<dtype>(i, (begin1 - hLen)) == labelRszd.at<dtype>(i, begin1)) &&
										 (labelRszd.at<dtype>(i, ((begin1 - 1))) == labelRszd.at<dtype>(i, begin1)))
										  ...
										  (labelRszd.at<dtype>(i, ((begin1 - 1))) != labelRszd.at<dtype>(i, begin1))))
								{
									...
									if (((end1 + hLen + 1) < depthIn.cols) && (labelRszd.at<dtype>(i, (end1 + hLen + 1)) == labelRszd.at<dtype>(i, begin1)))
									{
										...
										{
											if ((depthIn.at<dtype>(i, (end1 + 1 + rightInfoIndex)) == 0) || (labelRszd.at<dtype>(i, (end1 + 1 + rightInfoIndex)) != labelRszd.at<dtype>(i, (begin1))))
											{
												rightInfo = 0;
											}
										}
										...
										if (rightInfo == 1)
										{ // case 8  .. nothing on the left , more ethan enough on the right
											caseNo = 8;
											case8++;
										...
										else if (rightInfo == 0)
										{
											...
											if (rightInfo == 1)
											{ // case 9... nothing on the left, JUST enough on the right
												caseNo = 9;
												case9++;
											...
											else if (rightInfo == 0)
											{
												...
												if (depthIn.at<dtype>(i, (end1 + 1)) != 0)
												{ // case 10 .. nothing on the left, some but less than enough on the right
													caseNo = 10;
													case10++;
									...
									else if (((end1 + hLen + 1) < depthIn.cols) && (labelRszd.at<dtype>(i, (end1 + hLen + 1)) != labelRszd.at<dtype>(i, begin1)))
									{
										// there is NOT MORE than enough info on the right to fill the hole

										if (labelRszd.at<dtype>(i, (end1 + hLen)) == labelRszd.at<dtype>(i, begin1))
										{ // there is JUST enough info in the same label on the right
											...
											else if (rightInfo == 0)
											{
												...
												if (depthIn.at<dtype>(i, (end1 + 1)) != 0)
												{ // case 10 .. nothing on the left, some but not enough on the right
													caseNo = 10;
													case10++;
										...
										else if (labelRszd.at<dtype>(i, (end1 + 1)) == labelRszd.at<dtype>(i, begin1))
										{ // there is at least one pixel on the right but NOT ENOUGH in the same object
										...
										{ // there are NO pixels on the right in the same object
									...
									else if (((end1 + hLen + 1) > depthIn.cols))
									{ // the information on the right is not enough

										if (labelRszd.at<dtype>(i, (end1 + 1)) == labelRszd.at<dtype>(i, begin1))
										{ // there is at least one pixel on the right but NOT ENOUGH in the same object

											if (depthIn.at<dtype>(i, (end1 + 1)) != 0)
											{ // case 10 .. nothing on the left, some on the right but not enough
												caseNo = 10;
												case10++;
									...
									{ // the information on the right is JUST enough
					...
					if (caseNo == 0)
					{ // a condition in the "while" was not met and NO case number is assigned yet

						if ((depthIn.at<dtype>(i, j) != 0) && (labelRszd.at<dtype>(i, j) == labelRszd.at<dtype>(i, j + 1)))
						{ // the object has not end1ed but the hole has end1ed
							...
							{ // when there is enough information on the left of the hole
								...
								else if ((labelRszd.at<dtype>(i, ((begin1 - hLen) - 1)) != labelRszd.at<dtype>(i, begin1)) && (labelRszd.at<dtype>(i, ((begin1 - hLen))) == labelRszd.at<dtype>(i, begin1)) && (labelRszd.at<dtype>(i, ((begin1 - 1))) == labelRszd.at<dtype>(i, begin1)))
								{
									// and JUST enough information on the left is in the same object
									int checkLeftLabel = 1; // we assume the left info is all in the same label
								...
								{ // but ALL the information is NOT in the same object
									...
									{ // there is at least one pixel on the left in the same label
										...
										if (((end1 + hLen + 1) < depthIn.cols) && (labelRszd.at<dtype>(i, (end1 + hLen + 1)) == labelRszd.at<dtype>(i, begin1)) && (labelRszd.at<dtype>(i, (end1 + 1)) == labelRszd.at<dtype>(i, begin1)))
										{
											// there is more enough information on the right in the same label to fill the hole
											...
											if (rightInfo == 1)
											{ // case 6  .. not enough on the left , more ethan enough on the right
												caseNo = 6;
												case6++;
											...
											else if (rightInfo == 0)
											{
												...
												if (rightInfo == 1)
												{ // case 7... not enough on the left, JUST enough on the right
													caseNo = 7;
													case7++;
												...
												else if (rightInfo == 0)
												{
													...
													if (depthIn.at<dtype>(i, (end1 + 1)) != 0)
													{ // case 11 .. some on the left, some on the right, neither enough
														caseNo = 11;
														case11++;
										...
										{
											// there is NOT MORE than enough info on the right to fill the hole
											if ((labelRszd.at<dtype>(i, (end1 + hLen)) == labelRszd.at<dtype>(i, begin1)) && (labelRszd.at<dtype>(i, (end1 + 1)) == labelRszd.at<dtype>(i, begin1)))
							...
							{ //when there is NOT enough information on the left but some
								...
								{ // there is at least one pixel on the left in the same label
									...
									if (((end1 + hLen + 1) < depthIn.cols) && (labelRszd.at<dtype>(i, (end1 + hLen + 1)) == labelRszd.at<dtype>(i, begin1)) && (labelRszd.at<dtype>(i, (end1 + 1)) == labelRszd.at<dtype>(i, begin1)))
									{
										...
										if (rightInfo == 1)
										{ // case 6  .. not enough on the left , mor ethan enough on the right
											caseNo = 6;
											case6++;
								...
								{ // there are NO pixels on the left in the same label
									...
									if (((end1 + hLen + 1) < depthIn.cols) && (labelRszd.at<dtype>(i, (end1 + hLen + 1)) == labelRszd.at<dtype>(i, begin1)))
									{
										...
										if (rightInfo == 1)
										{ // case 8  .. nothing on the left , mor ethan enough on the right
											caseNo = 8;
											case8++;
						...
						else if ((depthIn.at<dtype>(i, j) == 0) && (labelRszd.at<dtype>(i, j) != labelRszd.at<dtype>(i, j + 1)))
						{ // the object has end1ed but the hole has NOT end1ed
							...
							{ // when there is enough information on the left of the hole
								...
								else if ((labelRszd.at<dtype>(i, ((begin1 - hLen) - 1))) != labelRszd.at<dtype>(i, begin1) && labelRszd.at<dtype>(i, (begin1 - hLen)) == labelRszd.at<dtype>(i, begin1))
								{
									// and Just enough information on the left is in the same object
									caseNo = 2;
									case2++;
								...
								{ // but ALL the information on the left is NOT in the same object
						...
						{ // the object has end1ed and the hole has end1ed
							...
							{ // when there is enough information on the left of the hole
								...
								else if ((labelRszd.at<dtype>(i, ((begin1 - hLen) - 1)) != labelRszd.at<dtype>(i, begin1)) && (labelRszd.at<dtype>(i, ((begin1 - 1))) == labelRszd.at<dtype>(i, begin1)) && (labelRszd.at<dtype>(i, ((begin1 - hLen))) == labelRszd.at<dtype>(i, begin1)))
								{
									// and JUST enough information on the left is in the same object
									int checkLeftLabel = 1; // we assume the left info is all in the same label
							...
							{ // when the amount of information on the left is EXACTLY the same as the length of the hole
								...
								if (labelRszd.at<dtype>(i, (begin1 - hLen)) == labelRszd.at<dtype>(i, begin1))
								{							// there is EXACTLY the same amount of info in the same label on the label
									int checkLeftLabel = 1; // we assume the left info is all in the same label
				...
				{ //when the hole begin1s at the very begin1ning of the line
					// which means nothing is available on the left

					while ((labelRszd.at<dtype>(i, j) == labelRszd.at<dtype>(i, j + 1)) && (depthIn.at<dtype>(i, j)) == 0)
					{
						j++;
					} // end1 of while

					if ((j == (depthIn.cols - 1)) || (labelRszd.at<dtype>(i, j) != labelRszd.at<dtype>(i, j + 1)))
					{ // the entire line from the very begin1ning of the LINE is made up of one object.
						// this means we stop when the object or the line is over

						if (depthIn.at<dtype>(i, j) == 0)
						{ //when the hole continues until the end1 of the line or label
							end1 = j;
							hLen = begin1 - end1 + 1;
						...
						{ // the object has not end1ed but the hole has end1ed
							...
							{ // the information on the right is JUST enough
								...
								{ // there is at least one pixel on the right but NOT ENOUGH in the same object
									...
									if (depthIn.at<dtype>(i, (end1 + 1)) != 0)
									{ // case 9 .. nothing on the left, some on the right, but not enough
										if (hLen == 1)
										{ // one pixel hole. nothing is available on the left and one pixel available on the right ... case 9
											caseNo = 9;
											case9++;
										}
										else
										{ // nothing on the left and some but not enough on the right .. case 10
											caseNo = 10;
											case10++;
										}
									} // end1 of case 10
				...
				{ // solving case 1
					...
					{
						if ((depthIn.at<dtype>(i, (cBegin1 + tindex)) == 0) || (labelRszd.at<dtype>(i, (cBegin1 + tindex)) != labelRszd.at<dtype>(i, begin1)))
							leftAllow = 0;
					}

					if (leftAllow == 1)
					{ // this means the info on th left is good so we do it

						for (int tindex = 0; tindex < hLen; tindex++)
						{

							depthIn.at<dtype>(i, (begin1 + tindex)) = depthIn.at<dtype>(i, (begin1 + tindex - 1)) +
																	  depthIn.at<dtype>(i, (cBegin1 + tindex)) -
																	  depthIn.at<dtype>(i, (cBegin1 + tindex - 1));

							if (depthIn.at<dtype>(i, (begin1 + tindex)) <= 1)
							{

								depthIn.at<dtype>(i, (begin1 + tindex)) = depthIn.at<dtype>(i, (begin1 + tindex - 1));
							}
							else if (depthIn.at<dtype>(i, (begin1 + tindex)) >= 255)
							{
							...
							else
							{
								...
								if (abs(depthIn.at<dtype>(i, (begin1 + tindex)) - depthIn.at<dtype>(i, (begin1 + tindex - 1)) > 10))
								{
				...
				{ // solving case 2
					...
					{ // this means the info on th left is good so we do it
						...
						if (hLen != 1)
						{
							...
							depthIn.at<dtype>(i, (begin1)) = depthIn.at<dtype>(i, (begin1 - 1));
				...
				{ // solving case 4
					...
					{ // this means the info on th left is good so we do it
						...
						if (hLen == 1)
							depthIn.at<dtype>(i, begin1) =
								(depthIn.at<dtype>(i, begin1 - 1) + depthIn.at<dtype>(i, begin1 + 1)) / 2;

						else
						{ // the hole is longer than one pixel

							// first assign the left most pixel in the hole
							depthIn.at<dtype>(i, begin1) = depthIn.at<dtype>(i, cEnd1) + depthIn.at<dtype>(i, cBegin1) - depthIn.at<dtype>(i, (cBegin1 - 1));
							// first assign the left most pixel in the hole
							...
							int diffH = (depthIn.at<dtype>(i, (end1 + 1)) - depthIn.at<dtype>(i, (begin1 - 1)));
							int diffC = (depthIn.at<dtype>(i, (cEnd1 + 1)) - depthIn.at<dtype>(i, (cBegin1 - 1)));
							double slope;

							int t1 = depthIn.at<dtype>(i, begin1 - 1);
							int t2 = depthIn.at<dtype>(i, end1 + 1);
							int t3 = depthIn.at<dtype>(i, cBegin1 - 1);
							int t4 = depthIn.at<dtype>(i, cEnd1 + 1);

							int upDownLineSolved = 0; // we first assume that the top cannot be used and then we check

							if ((i != 0) && (labelRszd.at<dtype>(i - 1, begin1 - 1) == labelRszd.at<dtype>(i, begin1 - 1)) &&
								(labelRszd.at<dtype>(i - 1, end1 + 1) == labelRszd.at<dtype>(i, end1 + 1)) &&
								(depthIn.at<dtype>(i - 1, begin1 - 1) == depthIn.at<dtype>(i, begin1 - 1)) &&
								(depthIn.at<dtype>(i - 1, end1 + 1) == depthIn.at<dtype>(i, end1 + 1)))
							{
								...
								for (int tindex = 0; tindex < hLen; tindex++)
								{
									if ((depthIn.at<dtype>(i - 1, begin1 + tindex) == 0) ||
										(labelRszd.at<dtype>(i - 1, begin1 + tindex) !=
										 labelRszd.at<dtype>(i, begin1 + tindex)) ||
										(i == 0))
									{
										upDownLineSolved = 0;
									}
								...
								if (upDownLineSolved == 1)
								{ // we can use and copy from the top line

									for (int tindex = 0; tindex < hLen; tindex++)
									{

										depthIn.at<dtype>(i, begin1 + tindex) = depthIn.at<dtype>(i - 1, begin1 + tindex);
									}

								} // end1 of if for when we can use and copy from the top line
							}

							else if ((i != depthIn.rows) && (labelRszd.at<dtype>(i + 1, begin1 - 1) == labelRszd.at<dtype>(i, begin1 - 1)) &&
									 (labelRszd.at<dtype>(i + 1, end1 + 1) == labelRszd.at<dtype>(i, end1 + 1)) &&
									 (depthIn.at<dtype>(i + 1, begin1 - 1) == depthIn.at<dtype>(i, begin1 - 1)) &&
									 (depthIn.at<dtype>(i + 1, end1 + 1) == depthIn.at<dtype>(i, end1 + 1)))
							{
								...
								if (upDownLineSolved == 1)
								{ // we can use and copy from the bottom line
							...
							if (((diffC == 0) && (diffH != 0) && (upDownLineSolved == 0)) || ((diffC == 1) && (diffH != 0) && (upDownLineSolved == 0)))
							{
								...
								for (int tindex = 0; tindex < hLen; tindex++)
								{
									if (abs(depthIn.at<dtype>(i, cBegin1 + tindex) - depthIn.at<dtype>(i, cBegin1)) > 1)
										//if(depthIn.at<dtype>(i,cBegin1 + tindex) != depthIn.at<dtype>(i,cBegin1))

										cFlat = 0;
								}

								if (cFlat == 1)
								{ // the sample area is definitely flat
									...
									int HoldFirstValue = depthIn.at<dtype>(i, begin1 - 1);
									...
									if (limit >= 1)
									{
										...
										for (int fIndex = 0; fIndex < diffH; fIndex++)
										{
											...
											for (int tindex = 0; tindex < limit; tindex++)
											{
												...
												depthIn.at<dtype>(i, (fIndex * limit) + (begin1 + tindex)) = HoldFirstValue;

											} // end1 of inner for

											if (HoldFirstValue < depthIn.at<dtype>(i, end1 + 1))
												HoldFirstValue++;
											else if (HoldFirstValue > depthIn.at<dtype>(i, end1 + 1))
												HoldFirstValue--;
							...
							if ((solved == 0 && times == 3 && (upDownLineSolved == 0)) || (solved == 0 && times == 2 && (upDownLineSolved == 0)))
							{
								...
								for (int tindex = -1; tindex < hLen + 1; tindex++)
								{ // moving the stuff from the image matrix to the temp array

									tempArray[cBegin1 + tindex] = (double)(depthIn.at<dtype>(i, cBegin1 + tindex));
								...
								for (int tindex = 0; tindex < hLen; tindex++)
								{ // moving the stuff back from the temp array to the image matrix

									depthIn.at<dtype>(i, (begin1 + tindex)) = (dtype)tempArray[begin1 + tindex];
				...
				{ // solving case 5
					...
					{ // this means the info on th left is good so we do it
						...
						{ // the hole is longer than one pixel
							...
							// first assign the left most pixel in the hole
							depthIn.at<dtype>(i, begin1) = depthIn.at<dtype>(i, cEnd1) + depthIn.at<dtype>(i, (cBegin1 + 1)) - depthIn.at<dtype>(i, cBegin1);
							// first assign the left most pixel in the hole

							int solved = 0; // for checking to see if holes have been filled. If it is zero, it means holes have not been solved yet

							// calculating the difference between the two sides of the hole and the two sides of the sample area
							int diffH = (depthIn.at<dtype>(i, (end1 + 1)) - depthIn.at<dtype>(i, (begin1)));
							int diffC = (depthIn.at<dtype>(i, (cEnd1 + 1)) - depthIn.at<dtype>(i, (cBegin1)));
							double slope;
							...
							if ((diffC == 0) && (diffH != 0) && (upDownLineSolved == 0))
							{ // the sample area may be flat but the hole is definitely not flat
								...
								{ // the sample area is definitely flat
									...
									int HoldFirstValue = depthIn.at<dtype>(i, cBegin1);
				...
				{ // solving case 6
					...
					if (rightAllow == 1)
					{ // this means the info on the right is good so we do it
						...
						{ // the hole is longer than one pixel
							...
							depthIn.at<dtype>(i, end1) = depthIn.at<dtype>(i, cBegin1) + depthIn.at<dtype>(i, cEnd1) - depthIn.at<dtype>(i, (cEnd1 + 1));
							// first assign the right most pixel in the hole
				...
				{ // solving case 7
					...
					{ // this means the info on the right is good so we do it
						...
						if (hLen == 1)
							depthIn.at<dtype>(i, begin1) = (depthIn.at<dtype>(i, begin1 - 1) + depthIn.at<dtype>(i, (begin1 + 1))) / 2;

						else
						{ // the hole is longer than one pixel

							// first assign the right most pixel in the hole
							depthIn.at<dtype>(i, end1) = depthIn.at<dtype>(i, cBegin1) + depthIn.at<dtype>(i, (cEnd1 - 1)) - depthIn.at<dtype>(i, (cEnd1));
							// first assign the right most pixel in the hole

							int solved = 0; // for checking to see if holes have been filled. If it is zero, it means holes have not been solved yet

							// calculating the difference between the two sides of the hole and the two sides of the sample area
							int diffH = (depthIn.at<dtype>(i, (end1)) - depthIn.at<dtype>(i, (begin1 - 1)));
							int diffC = (depthIn.at<dtype>(i, (cEnd1)) - depthIn.at<dtype>(i, (cBegin1 - 1)));
							double slope;
					...
					{ // this means the info on the right is good so we do it
						...
						if (hLen != 1)
							for (int tindex = 0; tindex < hLen; tindex++)
							{
								...
								depthIn.at<dtype>(i, (end1 - tindex)) = depthIn.at<dtype>(i, (end1 - tindex + 1)) +
																		depthIn.at<dtype>(i, (cEnd1 - tindex)) -
																		depthIn.at<dtype>(i, (cEnd1 - tindex + 1));

								if (depthIn.at<dtype>(i, (end1 - tindex)) <= 1)
								{

									depthIn.at<dtype>(i, (end1 - tindex)) = depthIn.at<dtype>(i, (end1 - tindex + 1));
								}
								else if (depthIn.at<dtype>(i, (end1 - tindex)) >= 255)
								{
								...
								{
									if (abs(depthIn.at<dtype>(i, (end1 - tindex)) - depthIn.at<dtype>(i, (end1 - tindex + 1)) > 10))
									{
							...
							depthIn.at<dtype>(i, (end1)) = depthIn.at<dtype>(i, (end1 + 1));
					...
					{ // this means the info on the right is good so we do it
						...
						if (hLen != 1)
							for (int tindex = 0; tindex < hLen; tindex++)
							{
								...
								depthIn.at<dtype>(i, (end1 - tindex)) = depthIn.at<dtype>(i, (cBegin1 - tindex)) +
																		depthIn.at<dtype>(i, (cEnd1 - tindex - 1)) -
																		depthIn.at<dtype>(i, (cEnd1 - tindex));
				...
				if (times == 3)
				{ // the desperate cases in the third run
					...
					{ //solving case 11
						...
						{ // if the hole is longer than one pixel

							// if for checking to see if cubic interpolation is possible
							if (((begin1 - 2) >= 0) && ((end1 + 2) < depthIn.cols) &&
								(labelRszd.at<dtype>(i, (begin1 - 2)) == labelRszd.at<dtype>(i, begin1)) &&
								(labelRszd.at<dtype>(i, (end1 + 2)) == labelRszd.at<dtype>(i, end1)))
							{

								double y0 = (double)depthIn.at<dtype>(i, (begin1 - 2));
								double y1 = (double)depthIn.at<dtype>(i, (begin1 - 1));
								double y2 = (double)depthIn.at<dtype>(i, (end1 + 1));
								double y3 = (double)depthIn.at<dtype>(i, (end1 + 2));
								...
								for (double interi = 1; interi <= hLen; interi++)
								{
									...
									depthIn.at<dtype>(i, (begin1 - 1 + interi)) = (a0 * (coeff * interi) * mu) + (a1 * mu) + (a2 * (coeff * interi)) + a3;
							...
							{ // we perform the linear interpolation
								...
								for (double interi = 1; interi <= hLen; interi++)
									depthIn.at<dtype>(i, (begin1 - 1 + interi)) = (int)((y1 * (1 - (interi * coeff))) + (y2 * (interi * coeff)));
					...
					else if ((caseNo == 3) && (times == 3))
					{ //solving case 3

						if ((labelRszd.at<dtype>((i - 1), (begin1 - 1)) == labelRszd.at<dtype>(i, (begin1 - 1))) &&
							(labelRszd.at<dtype>((i - 1), end1) == labelRszd.at<dtype>(i, end1)) &&
							(i != 0))
						{ // checking to see if the upper line is available
							...
							for (int tindex = 0; tindex < hLen; tindex++)
							{
								...
								depthIn.at<dtype>(i, (begin1 + tindex)) = depthIn.at<dtype>((i - 1), (begin1 + tindex)) + depthIn.at<dtype>((i), (begin1 + tindex - 1)) -
																		  depthIn.at<dtype>(i - 1, (begin1 + tindex - 1));
						...
						else if ((labelRszd.at<dtype>(i, (begin1 - 2)) == labelRszd.at<dtype>(i, begin1)) &&
								 (depthIn.at<dtype>(i, (begin1 - 2)) != 0))
						{ // checking to see if TWO pixels on the left are available

							for (int tindex = 0; tindex < hLen; tindex++)
							{

								depthIn.at<dtype>(i, (begin1 + tindex)) = depthIn.at<dtype>(i, (begin1 - 1 + tindex)) + depthIn.at<dtype>(i, (begin1 + tindex - 2)) -
																		  depthIn.at<dtype>(i, (begin1 + tindex - 1));
						...
						{ // this happens when only one pixel is available on the left

							for (int tindex = 0; tindex < hLen; tindex++)
								depthIn.at<dtype>(i, (begin1 + tindex)) = depthIn.at<dtype>(i, (begin1 - 1 + tindex));
					...
					{ //solving case 10
						...
						if ((i != 0) && (labelRszd.at<dtype>((i - 1), (end1)) == labelRszd.at<dtype>(i, (end1))) &&
							(labelRszd.at<dtype>((i - 1), begin1) == labelRszd.at<dtype>(i, begin1)) &&
							(depthIn.at<dtype>((i - 1), (end1 + 1)) == depthIn.at<dtype>((i), (end1 + 1))))
						{ // checking to see if we can directly copy from the upper line
							...
							for (int tindex = 0; tindex < hLen; tindex++)
							{
								if ((depthIn.at<dtype>((i - 1), (begin1 + tindex)) == 0) || (labelRszd.at<dtype>((i - 1), (begin1 + tindex)) != (labelRszd.at<dtype>((i), (begin1 + tindex)))))
						...
						else if ((i != 0) && (labelRszd.at<dtype>((i - 1), (end1 + 1)) == labelRszd.at<dtype>(i, (end1 + 1))) &&
								 (labelRszd.at<dtype>((i - 1), begin1) == labelRszd.at<dtype>(i, begin1)) && (labelRszd.at<dtype>((i - 1), (end1)) == labelRszd.at<dtype>((i - 1), (end1 + 1))))
						{
							...
							if (upLine == 1)
							{
								...
								for (int tindex = 0; tindex < hLen; tindex++)
								{
									...
									depthIn.at<dtype>(i, (end1 - tindex)) =
										depthIn.at<dtype>((i - 1), (end1 - tindex)) +
										depthIn.at<dtype>((i), (end1 - tindex + 1)) -
										depthIn.at<dtype>(i - 1, (end1 - tindex + 1));
						...
						else if (((end1 + 2) < depthIn.cols) && (labelRszd.at<dtype>(i, (end1 + 2)) == labelRszd.at<dtype>(i, end1)) &&
								 (depthIn.at<dtype>(i, (end1 + 2)) != 0))
						{ // checking to see if TWO pixels on the right are available

							for (int tindex = 0; tindex < hLen; tindex++)
							{

								depthIn.at<dtype>(i, (end1 - tindex)) = depthIn.at<dtype>(i, (end1 + 1 - tindex)) + depthIn.at<dtype>(i, (end1 - tindex + 2)) -
																		depthIn.at<dtype>(i, (begin1 - tindex + 1));
						...
						{ // this happens when only one pixel is available on the right

							for (int tindex = 0; tindex < hLen; tindex++)
								depthIn.at<dtype>(i, (end1 - tindex)) = depthIn.at<dtype>(i, (end1 + 1 - tindex));
					...
					{ //solving case 12
						...
						if ((labelRszd.at<dtype>((i - 1), end1) == labelRszd.at<dtype>(i, end1)) &&
							(labelRszd.at<dtype>((i - 1), begin1) == labelRszd.at<dtype>(i, begin1)) &&
							(i != 0))
						{ // checking to see if the upper line is available
